...
1. 点击项目按钮打开“选择本机项目”。
2. Windows 上点击“Windows 文件夹”，在系统文件夹窗口中选择项目目录；选择完成后 AOD 会自动回填路径并验证。
3. 也可以从已注册列表选择、浏览主机目录，或直接输入绝对路径并点击“验证”。
4. 确认 Git 根、分支、commit 和 clean/dirty 状态后，点击“注册并选择”。

Windows 文件夹选择器由本机 AOD 服务调用，仅在服务和 Chrome 位于同一台 Windows 主机时可用。取消系统窗口不会改变当前选择；如果服务不是 Windows，仍可使用路径输入和 AOD 目录浏览器。
...
运行中心的“发起工作”把三种创建路径集中在一个窗口，并始终显示当前绑定项目：

- **群组讨论**：选择已有 Agent 群组，创建会话；勾选“创建后立即开始讨论”即表示本次人工确认启动 Agent。
- **智能规划**：选择规划器生成可编辑 DAG，确认预览后才创建运行和 worktree。
- **直接任务**：跳过讨论和 DAG，为一个 Agent 创建边界明确的独立任务。

“快速任务”和任务队列中的“发起任务”会直接打开统一发起器的“直接任务”模式。发起过程中窗口会持续显示当前阶段和已等待时间；长时间没有正文输出不再表现为页面卡死。

### 7.1 创建单个任务

适合范围明确、不需要先规划 DAG 的工作。

1. 点击“快速任务”，或打开“发起工作”并选择“直接任务”。
2. 描述要完成的工作；任务标题可留空，默认使用需求第一行。
3. 选择执行 Agent。
