...
- **运行中心**：查看指标、群组、任务、运行和事件。
- **Agent 群组**：定位到群组模板和群组会话。
- **任务队列**：定位到普通任务与角色任务。
- **GitHub 交付**：定位到运行和 PR 状态。

导航可以收起为图标栏，偏好会保存在浏览器本地。
...
2. 输入群组名称和描述。
3. 启用至少两个成员。
4. 为成员选择角色：`executor`、`reviewer`、`fixer` 或 `advisor`。
5. 指定一名主持者。
6. 设置最大修复次数。
7. 保存群组。
...
- 群组必须包含执行者和检查者。
- 最大修复次数大于 0 时必须包含修复者。
- 当前版本中，同一个群组内每种 Agent 适配器只能加入一次。
- 主持者可以兼任成员的主角色。
